...
struct basic_block* create_test_block(struct ir_context *ctx, struct basic_block *prev) {
    struct basic_block *block = block_new(ctx);
    struct inst_jump *jmp = inst_new_jump(ctx, block);
    block_insert(prev, &jmp->inst);
}

struct basic_block* create_test_cfg(struct ir_context *ctx) {
    int i = 0;
    struct basic_block *prev = block_new(ctx);
    prev->value.name = format_range("{int}", i + 1);
    
    struct basic_block *result = prev;
    for (i = 1; i < 5; i++) {
        struct basic_block *nblock = block_new(ctx); 
...
int main(int argc, char *args[]) {
    ...
    struct basic_block *entry = block_new(&ctx);
    entry->value.name = RANGE_STRING("entry"); 
    struct basic_block *loop = block_new(&ctx);
    loop->value.name = RANGE_STRING("loop"); 
    struct basic_block *loop2 = block_new(&ctx);
...
//    struct basic_block *entry = create_test_cfg(&ctx);
    struct function *fun = ir_new_function(&ctx, RANGE_STRING("test"), entry);
