...
void arch_functionCall(struct codegen *cg, label_t *label, size_t count, struct variable **var) {
    ...
    for (int i = 0; i < 6; i++) {
        // if the variable is already on the correct register then don't mess with it.
        
        struct variable *regVar = cg->registerStatus[regs[i]];
        
                // This register is caller saved, so save it.
        if (i < count) {
            // If there is no register allocated to this variable then load id to the correct one.
            if (var[i]->reg == -1) {
                var[i]->reg = regs[i];
            ...
            emit_storeReg64(&cg->buffer, _getRealReg(var[i]->reg), _getRealReg(regs[i]));
        }
    }
