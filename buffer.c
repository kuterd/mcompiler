...
void dbuffer_ensureCap(dbuffer_t *dbuffer, size_t size) {
    if (size > dbuffer->capacity - dbuffer->usage) {
        size_t newSize =
            max(dbuffer->capacity * 2, dbuffer->capacity * 3 / 2 + size);
...
void dbuffer_removeRange(dbuffer_t *dbuffer, size_t offset, size_t size) {
    ...
    memmove(dbuffer->buffer + offset, dbuffer->buffer + offsetEnd,
            dbuffer->usage - offsetEnd);
...
void dbuffer_popPtr(dbuffer_t *dbuffer) { dbuffer->usage -= sizeof(void *); }
...
void dbuffer_clear(dbuffer_t *dbuffer) { dbuffer->usage = 0; }

void dbuffer_free(dbuffer_t *dbuffer) { free(dbuffer->buffer); }
